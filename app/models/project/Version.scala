...
/**
  ...
  */
case class Version(id: Option[Int] = None,
  ...
  override def setVisibility(visibility: Visibility, comment: String, creator: Int)(implicit ec: ExecutionContext, service: ModelService): Future[(Version, VersionVisibilityChange)] = {
    ...
    val createNewChange = service.access(classOf[VersionVisibilityChange]).add(
      VersionVisibilityChange(None, Some(Timestamp.from(Instant.now())), Some(creator), this.id.get, comment, None, None, visibility.id)
    )
  ...
  def unfinishedReviews(implicit ec: ExecutionContext, service: ModelService): Future[Seq[Review]] = reviewEntries.all.map(_.toSeq.filter(rev => rev.createdAt.isDefined && rev.endedAt.isEmpty).sortWith(byCreationDate))
  ...
  def reviewById(id: Int)(implicit ec: ExecutionContext, service: ModelService): OptionT[Future, Review] = reviewEntries.find(equalsInt[ReviewTable](_.id, id))
