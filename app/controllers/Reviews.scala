      ...
      val ret = for {
        ...
        _ <- {
          ...
          val closeOldReview = version.mostRecentUnfinishedReview.semiFlatMap { oldreview =>
            service
              .update(oldreview.copy(endedAt = Some(Timestamp.from(Instant.now()))))
              .flatMap(_.addMessage(Message(message.trim, System.currentTimeMillis(), "takeover")))
              .flatMap(_ => this.service.insert(Review(Some(1), Some(Timestamp.from(Instant.now())), version.id.get, request.user.id.get, None, "")))
              .map(_ => ())
          }.getOrElse(())
