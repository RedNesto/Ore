...
class Versions @Inject()(stats: StatTracker,
  ...
  /**
    * Un deletes the specified version.
    *
    * @param author Project owner
    * @param slug   Project slug
    * @return Home page
    */
  def unDelete(author: String, slug: String, versionString: String): Action[AnyContent] = {
    (Authenticated andThen PermissionAction[AuthRequest](ReviewProjects)).async { implicit request =>
      ...
      val res = for {
        ...
        UserActionLogger.log(request, LoggedAction.VersionDeleted, version.id.getOrElse(-1), s"UnDelete: ${comment}", "")
        Redirect(self.showList(author, slug, None))
      }
      res.merge
    }
