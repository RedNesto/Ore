    ...
    val res = for {
      ...
      res <- {
        if (tagline.length > maxLen)
          ...
          val log = UserActionLogger.log(request, LoggedAction.UserTaglineChanged, user.id.get, tagline, user.tagline.getOrElse("null"))
  ...
  def deletePgpPublicKey(username: String, sso: Option[String], sig: Option[String]): Action[AnyContent] = {
    VerifiedAction(username, sso, sig).async { implicit request =>
      ...
      if (user.pgpPubKey.isEmpty)
        ...
        val log = UserActionLogger.log(request, LoggedAction.UserPgpKeyRemoved, user.id.get, "", "")
